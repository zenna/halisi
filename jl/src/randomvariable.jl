...
rv_push(x, ω) = if isa(x, RandomVariable) x(ω) else x end
...
# Overloads rv[1,2]
getindex(rv::RandomArray, i::Integer, j::Integer) = ω->rv_push(rv(ω)[i,j],ω)

## Functionally set a value in a random array
## This is inefficient
function setindex{T}(rv::RandomArray,v::T,i::Integer,j::Integer)
  ω->(a = rv(ω); [if (i == n) && (j == p) rv_push(v,ω) else rv_push(a[n,p],ω) end
