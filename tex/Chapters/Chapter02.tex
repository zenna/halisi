...
% We wish to construct a probabilstic program which when evaluated draws samples from a conditonal distribution $P^*(x) = P(x \vert C^*)$.
...
The extraction of this logical expression from the program is important, and the first step of the proposed method.
...
One application of constrained inference is a straightforward generalisation of the problem statement: from all possible paths within a region, sample a .
More formally we seek to construct a path $\{x_0,y_0,x_1,y_1,..,x_n,y_n\}$.
Obstacles can be defined as polyhedra, but for simplicity and without loss of generality we will assume them to be isothetic (axis aligned) rectangles or boxes, and represented in h-space.
...
This is a non-linear problem, which can be seen intuively by observing that the constraints on any point depend on the previous point.
...
I suggest this can be overcome by incorporating two extra details.  First we ensure that the maximum edge length is within some radius.
This is a linear problem.  Then we increase the number of points.
...
In a functional programming language, we get:
Note one power of using a programming languages, our program is now of constant length  

Hence the trade-off is severe.
For every new point we add we will gain two new dimensions, and described in more detail in the following paragraph, you'll get b more polytopes.
...
Program analysis
if or and functions primitive recursion
symbolic exection
